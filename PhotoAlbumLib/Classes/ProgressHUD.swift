...
//  wutong
//
//  Created by Jeejio on 2021/1/16.
//
import UIKit
...
public class ProgressHUD: UIView {
    ...
    @objc public func show(timeout: TimeInterval = 100) {
        ...
        if timeout > 0 {
            self.cleanTimer()
            self.timer = Timer.scheduledTimer(timeInterval: timeout, target: WeakProxy(target: self), selector: #selector(timeout(_:)), userInfo: nil, repeats: false)
